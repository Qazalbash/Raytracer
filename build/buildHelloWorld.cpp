...
void World::build(void) {
        ...
        // sphere
        Sphere* sphere_ptr = new Sphere(Point3D(-3.0f, 2.0f, 0.0f), 5.0f);
        sphere_ptr->set_material(new Cosine(red));
        add_geometry(sphere_ptr);

        // triangle
        Point3D   a(2.5f, -5.0f, 1.0f);
        Point3D   b(14.0f, -1.0f, 0.0f);
        Point3D   c(8.5f, 5.0f, 0.5f);
        Triangle* triangle_ptr = new Triangle(a, b, c);
        triangle_ptr->set_material(new Cosine(blue));
        add_geometry(triangle_ptr);

        // plane
        Plane* plane_ptr = new Plane(Point3D(0.0f, 1.0f, 0.0f), Vector3D(0.0, 10.0, 2.0));
