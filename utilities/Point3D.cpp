 ...
 * @return Vector3D
 */
Vector3D Point3D::operator-(const Point3D& p) const { return Vector3D(this->x - p.x, this->y - p.y, this->z - p.z); }
 ...
 * @return Point3D
 */
Point3D Point3D::operator+(const Vector3D& v) const { return Point3D(this->x + v.x, this->y + v.y, this->z + v.z); }
 ...
 * @return Point3D
 */
Point3D Point3D::operator*(const float s) const { return Point3D(this->x * s, this->y * s, this->z * s); }
 ...
 * @return float
 */
float Point3D::d_squared(const Point3D& p) const { return pow((x - p.x), 2) + pow((y - p.y), 2) + pow((z - p.z), 2); }
 ...
 * @return float
 */
float Point3D::distance(const Point3D& p) const { return sqrt(Point3D::d_squared(p)); }
