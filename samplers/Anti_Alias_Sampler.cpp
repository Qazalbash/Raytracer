...
std::vector<Ray> Anti_Alias_Sampler::get_rays(int px, int py) const {
    ...
    for (int i = 0; i < 100; i++) {
        pixelH = (viewplane_ptr->bottom_right.y - viewplane_ptr->top_left.y) / viewplane_ptr->vres,
        pixelW = (viewplane_ptr->bottom_right.x - viewplane_ptr->top_left.x) / viewplane_ptr->hres;
