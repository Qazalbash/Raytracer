...
std::vector<Ray> Simple::get_rays(int px, int py) const {
    ...
    const float pixelH = (viewplane_ptr->bottom_right.y - viewplane_ptr->top_left.y) / viewplane_ptr->vres;
    const float pixelW = (viewplane_ptr->bottom_right.x - viewplane_ptr->top_left.x) / viewplane_ptr->hres;
